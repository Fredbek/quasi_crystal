...
def main():
    ...
    for Length in Lengths:
        ...
        ax.scatter(X_bd, Y_bd, label = "Length: " + meio, color = colors[m], marker = markers[m])
        ax.errorbar(X_bd, Y_bd, Yer_bd, color = colors[m], marker = markers[m], ls = 'dotted')
    ...
    Y = np.asarray(np.arange(0.38, 1.2 + 0.01, 0.01))
    ...
    ax.plot(X, Y, ls = '-.', color = 'black', label = 'Critical Temp')

    ax.set_xlim = (2.3, 2.4)
    ax.set_ylim = (0.38, 1.02)

    ax.set_xlabel(r'$k_B\cdot T\cdot J$', size = 14)
    ax.set_ylabel(r'$B_4$', size = 14)

    ax.set_title('Binder\'s cumulant X Temperature', size = 16)
